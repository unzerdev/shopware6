...
class UnzerApplePayV2PaymentHandler extends AbstractUnzerPaymentHandler
{
    ...
    public function pay(
        ...
        if (!empty($currentRequest->getSession()->get(ExpressCheckoutService::SESSION_APPLEPAY_PAYMENT_TYPE_ID))) {
            $client = $this->clientFactory->createClientFromSalesChannelId($salesChannelContext->getSalesChannelId(), $currentRequest);
            $this->paymentType = $client->fetchPaymentType($currentRequest->getSession()->get(ExpressCheckoutService::SESSION_APPLEPAY_PAYMENT_TYPE_ID));
            $this->isExpress = true;
        }
